...
\todo[inline]{Co jsme doposud zjistili, k cemu je to dobre a co s tim ted budeme delat. Co bude v teto kapitole. ze jsme zatim probrali teorii a podobne situace na ktere byla aplikovana, a ze v teto kapitole zkusime vlastni jiny navrh.}
\todo[inline,color=magenta]{Popsat ze se ty veci udelaly a uz jsou hotove, ale neresit jak se to implementovalo a konkretne spoustelo.}

\section{Overall architecture}
\todo[inline]{Jak vypada celkovy navrh modelu, velky diagram.}
\begin{figure}[!t]
	\centering
	\includegraphics[width=0.5\textwidth]{./fig/proposed-model.pdf}
...
This chapter describes methods for implementing neural network models.

mage captioning model was proposed and designed. 

\todo[inline]{Pridat text.}

\section{Tools}\label{chp:exper}

In this section I will describe tools and frameworks, which are used to implement deep learning models. \emph{Torch} will be discussed separately in its own section as it was a tool I used for experimenting with language modeling - predicting next character in text, which is second part of this chapter.
