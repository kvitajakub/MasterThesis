...
Uvod,graf s delkou sekvenci, kvuli nizkemu poctu dlouhych sekvenci nema cenu pracovat s errory po 70-80 znaku

komentare k tomu co bude v kapitole

jak to fungovalo a jak ne a proc komentar v kazde sekci

souhrn v dalsi kapitole?

\begin{figure}[!b]
	\centering
	\includegraphics[width=0.6\textwidth]{fig/sequence_length_distribution.pdf}
	\caption{Distribution of caption lengths in the training data.
		\label{fig:length}}
\end{figure}

\section{Pretraining \gls{rnn}}

ukazky generovani a graf chyb

\todo[inline]{kolik epoch jsem trenoval}

\begin{figure}[!t]
	\centering
	\includegraphics[width=0.6\textwidth]{fig/error_pretraining.pdf}
	\caption{Error based on character position in sequence in pretrained \gls{rnn}s.
