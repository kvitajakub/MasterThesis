...
\todo[inline]{Odstavec o vanishing gradientu, reseni pomoci long short-term memory, ktere bude dale.}

unrolling on long sequences an creation of very deep neural networks manifested the problem of vanishing or exploding gradient... neco o tom, zakoncit to s lstm.
\todof{Predelat}It also shows that, as very deep networks, they have vanishing or exploding gradient problem, which means that the network is not able to learn long-term dependencies, even though in theory it should. This is a serious issue, which is caused by iterating many times over the weights and the activation function with derivatives $>1$ (exploding gradient) or $<1$ \todof{Vice rozpracovat?}(vanishing gradient). Gradient then dies out and learning stops for distant dependencies. Among others this problem has been solved by the Long Short-Term Memory described in part \ref{subsubsec:lstm}.

\todo[inline]{Popis co bude v teto sekci a druha cast o language modelech atd..}

	\subsection{Recurrent units}\label{subsec:recUnits}

	\todo[inline]{Nejaky uvod o tom ze jsou ruzne druhy neuronu... Na zacatku se pouzivaly jednoducha vanilla rekurentni architektura, ale mely sve problemy, velmi spatne se trenovaly. pozdeji byly navrzeny komplikovanejsi architektury z nichz v soucasnosti dominuje lstm a jeji derivaty jako je gru, ktere budu popisovat v teto casti. tyto architektury udelaly praci s RNN a jejich trenovani mnohem jednodussi a lehci.}
	%reference? https://www.reddit.com/r/MachineLearning/comments/2ppojv/implementing_a_deep_lstmrnn_%C3%A0_la_sequence_to/
