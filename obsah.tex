...
Original network produces probability distribution over 1000 ImageNet classes. For my purposes, I removed ...
\todo[inline]{Popsat co jsem odstranil z CNN, co dale pouzivam.}

The image encoding produced by \gls{cnn} has to be preprocessed before entering the \gls{rnn}. For this purpose, several feed-forward layers in the \emph{Adapter} are present, between \gls{cnn} and \gls{rnn}. \emph{Adapter} consists of three fully connected Linear layers and two evaluations of hyperbolic tangens in the middle. As the \gls{cnn} does not have to be finetuned for the task, this should provide basic ability to adapt the encoding to current purpose and to give the vector the proper size, as required by hidden state initialization algorithm, described in section \ref{subsec:initialization}.
