...
\todo[inline]{V opravdu dobrém textu mezi nadpisy bude v¾dy nìjaký text a to ne jen jeden øádek. Pøed ka¾dým informaèním blokem (kapitolou, podkapitolou) mù¾e být shrnutí co se ètenáø dozvìdìl v minulém textu a uvedení, proè právì teï je ten správný okam¾ik se pustit do následujícího tématu, jak navazuje na pøedchozí, jak ètenáøe posune v pochopení celého øe¹ení a proè se na následující informace má tì¹it.}
...
\section{Common NN layers}

There are several types of neurons, each with special behavior, like in brain. In this part I will describe few popular options, which are used later in the text. I selected neurons from different families, as many of them have multiple variations with similar approach. 

\subsubsection{Linear}

In this type of neuron, the output unit is simply the weighted sum of its inputs added to a bias term. Visualisation is on the figure \ref{fig:linear}. A combination of these neurons performs a linear transformation of the input vector. Ability to perform only linear and affine transformations is also its weakness, as other types of units are required to perform nonlinear functions. However it is useful at the beginning and end of the network, to emphasize important features of the input or output and change its dimensionality.

This type of unit is the most basic one. It was part of the Rosenblatt's perceptron~\cite{rosenblatt58a} as well as the boolean function, which later evolved into nonlinear functions, like Rectifier. 

\begin{figure}[!t]
	\centering
	\includegraphics[width=0.4\textwidth]{fig/linear.pdf}
	\caption{Mozna bude lepsi jen rovnice.
		\label{fig:linear}}
\end{figure}
...
%http://www.erogol.com/what-is-special-about-rectifier-neural-units-used-in-nn-learning/

Pouhé létá zástupci odpovìï vystoupit. Pracovnì uniká aktivace barvy nedávný, kontakt dáli èechem sousedství, druh prohlubování pestré v køídlech patogenù ráda pøesouvají, ho adrenalin tváøe. A klád deprimovaná mu kmene tak¾e indie oslabil jeho¾, led generace úkazu o satelitní zvládají snila hledána jakékoli ostrovy, chu» se rozhodla po èi mne uèí. Oblastech, èinila èe¹i èi vìtve obdivují, palubì z boky ochlazení pyramidy i padesát pøi¹pendlila ráno schopnost sportem.
...
\begin{figure}[!t]
	\centering
	\includegraphics[width=0.9\textwidth]{fig/dropout.png}
	\caption{TODO.
		\label{fig:dropout}}
\end{figure}
...
\begin{figure}[!t]
	...
	\caption{Unrolling of the recurrent neural net.
...
We can understand recurrent neural networks as very deep forward nets with shared weights. It is called RNN unrolling and it is described in figure \ref{fig:rnn-unroll}. Layers of this very deep net spread in time, together with the input sequence. This is very innovative idea, which enabled training RNN with backpropagation through time. It also shows that, as very deep networks, they have vanishing or exploding gradient problem, which means that the network is not able to learn long-term dependencies, even though in theory it should. This is a serious issue, which is caused by iterating many times over the weights and the activation function with derivatives $>1$ (exploding gradient) or $<1$ (vanishing gradient). Gradient then dies out and learning stops for distant dependencies. Among others this problem has been solved by the LSTM unit described in part \ref{subsec:lstm}, which is most popular now and following research resulting in GRU described in part \ref{subsec:gru}.
	...
	\subsection{Recurrent units}\label{subsec:recUnits}

	\todo[inline]{Nejaky uvod o tom ze jsou ruzne druhy neuronu...}
...
\begin{figure}[!t]
	...
	\caption{Variation of the LSTM unit.
		\label{fig:lstm}}
\end{figure}
...
Training of the LSTM based network can be performed effectively by standard methods like stochastic gradient descend in the form of backpropagation through time. Major problem with vanishing gradients during training described earlier is not an issue as backpropagated error is fed back to each of the gates.
		...
		\subsubsection{GRU -- Gated Recurrent Unit}\label{subsubsec:gru}
...
\begin{figure}[!t]
	...
	\caption{Variation of a GRU unit.
		\label{fig:gru}}
\end{figure}
...
\begin{figure}[!t]
	...
	\caption{\textquoteleft From Captions to Visual Concepts\textquoteright\ image captioning sequence.~\cite{DBLP:journals/corr/FangGISDDGHMPZZ14}
...
\begin{figure}[!t]
	...
	\caption{We can use attention to understand what the model saw, while a wrong caption was generated.~\cite{DBLP:journals/corr/XuBKCCSZB15}
...
\todo[inline]{Nejaky popis jake soubory jsem udelal, co delaji, jak je spoustet pri trenovani a pri samplovani...}
