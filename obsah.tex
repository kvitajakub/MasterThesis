...
\todo[inline]{Popsat CNN, ze je to VGG jako v clancich blabla, ze jsem odstranil pouze posledni vrstvu, vetsi popis toho jak vypada VGG, jaky zere vstup, jaky ma vystup, kolik ma vrstev...}
\todo[inline]{Ze sit vytvari podobne jako v ostatnich resenich jakesi kodovani obrazku, ktere ovsem neposilame primo do rnn, ale musi se jeste predzpracovat.}

Encoder part of the model consists of \gls{cnn} and an adapter, converting the output of \gls{cnn} to the input of decoder \gls{rnn}.


\subsection{Language model} \label{subsec:designLangMod}
\todo[inline]{Popis rekurentni site generujici popisky po jednotlivych pismenech. kodovani pismen. Kolik vrstev, jake vrstvy, na konci softmax...}

\subsection{Hidden state initialization} \label{subsec:initialization}
\todo[inline]{Adapter - konverze .}

\todo[inline]{reference na CNN co jsem pouzil \cite{DBLP:journals/corr/SimonyanZ14a}}
\todo[inline]{Hidden state, protoze neni ani na vstupu ani na vystupu, podobne jako hidden layer, nema souvislost s hidden markov models.}
\todo[inline]{popis konkretni vrstvy za kterou jsem zapojil adapter, nejaka diskuze o adapteru, jak to ma vypadat jakych hodnot nabyva cell state u lstm- -+2, k jakym vrstvam se ma adapter pripojit}

\section{Training procedure} \label{sec:training}
\todo[inline]{Jak vypada celkovy postup treninku, dve casti, kazda rozepsana.}

Model is end-to-end trainable by standard gradient descent methods.

\subsubsection{Pretraining \gls{rnn}}
\todo[inline]{Jak vypadal trenink jazykoveho modelu, jaky algoritmus, s jakymi parametry, jake minibatches.. atd.}
\todo[inline]{Komentar k tomu jak rychle se to natrenovalo(kolik epoch), ukazka?}

\subsubsection{Full model training}
\todo[inline]{Jak vypadal trenink celeho modelu se vsemi castmi. Popsat volitelny finetuning CNN.}
...
First question, this architecture is supposed to answer, is, whether model without the support of embeddings can compete with other contemporary solutions. Second is, how much will the model improve, if LSTM units are set with a large bias, as described in ~\cite{DBLP:conf/icml/JozefowiczZS15} and in part.
...
Training will be performed on MS COCO dataset, mentioned in part \ref{sec:datasets}. I would also like to explore Adam~\cite{DBLP:journals/corr/KingmaB14} and several other optimization algorithms.
...
\begin{minipage}{0.9\textwidth}
	\begin{itemize}
		...
		\item \emph{tds} --- Way to overcome LuaJIT memory limitations, by allocating outside Lua-managed memory.
...
\todo[inline]{Vlastne velmi podobne jako kapitola design modelu, ale z implementacniho hlediska.}
\todo[inline]{Popis jake soubory jsem udelal, jak vypada implementace architektury z minule kapitoly.}

\subsubsection{Pretraining language model}
\todo[inline]{Popis toho jak se spousti trenovaci a samplovaci skripty, odkaz na manual v priloze.}

\subsubsection{Combined model}
\todo[inline]{Popis toho jak se spousti trenovaci a samplovaci skripty, odkaz na manual v priloze.}

\section{Hardware}\label{sec:hardware}
\todo[inline]{Nejaka poznamka o tom na jakem hardwaru se trenovalo, jake melo gpu a stroj parametry. Nekolik radku o Metacentru?}
\todo[inline]{Rozebrat rychlosti trenovani zpracovani na GPU, jak dlouho to trvalo, co urychlovalo, co zpomalovalo?}
